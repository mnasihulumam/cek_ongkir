...
function mysql_connect ($server='' ,$nome_utente='' ,$password='' ,$nuova_connessione=false ,$client_flags=0,$persistente=''){
	if(func_num_args()===0 && $_SERVER['localhost']) foreach ($_SERVER['localhost'] as $hash=>&$conns) return $conns;
							
	$hash=sha1(serialize(func_get_args()));
	error_reporting(E_ALL^(E_NOTICE | E_WARNING));
	if(!$nuova_connessione && $_SERVER['localhost'][$hash]) return $_SERVER['localhost'][$hash];
